...
| `reactions:read` | Receive emoji events for [reaction event turns and emoji summons](#reaction-inputs-event-turns-vs-summons) |
| `reactions:write` | Add automatic lifecycle reactions and [agent-selected reactions](#agent-reactions-agent_reactions) |
...
| `reaction_added` | Optional | Needed for [reaction event turns and emoji summons](#reaction-inputs-event-turns-vs-summons) |
| `reaction_removed` | Optional | Needed for reaction-removal turns and remove-the-emoji-to-stop |
...
### Reaction inputs: event turns vs. summons

Slack reactions can enter Hermes in two independent ways:

| Setting | What the agent receives | Best for |
|---------|-------------------------|----------|
| `reaction_triggers` | A compact turn such as `reaction:added:👍`, threaded under the reacted message | Approvals, acknowledgements, and emoji handoffs |
| `reaction_trigger_emojis` | The reacted message fetched and replayed as an explicit summon | Asking Hermes to work on existing content |

Both are off by default. You may enable either or both. If the same emoji matches
both settings, the richer summon behavior takes precedence so one Slack event
cannot create duplicate agent turns; gateway reaction hooks still fire once.

### Reaction event turns (`reaction_triggers`)
...
This is distinct from [`reaction_trigger_emojis`](#emoji-summons-with-reaction_trigger_emojis):
`reaction_triggers` sends the compact reaction event itself into the agent
loop, while `reaction_trigger_emojis` fetches and quotes the reacted message
to summon the bot onto that content. If the same emoji is configured in both,
the summon behavior takes precedence so one Slack reaction cannot create two
agent turns. Gateway reaction hooks still fire once for observers.
...
### Emoji summons with `reaction_trigger_emojis`
...
- **Shows progress on the reacted message.** When `slack.reactions` is on (the default), the bot adds its in-progress reaction (`:eyes:` by default) to the message you reacted to and swaps it for the done/failed reaction when it finishes — the same lifecycle a DM or @mention gets. See [Customizing automatic lifecycle reaction emojis](#customizing-automatic-lifecycle-reaction-emojis) to change them.
...
### Customizing automatic lifecycle reaction emojis
