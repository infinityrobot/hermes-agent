...
from typing import Callable, ClassVar, Dict, Optional, Any, Tuple, List, NamedTuple
...
class SlackAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Most-recent inbound (real) message ts per workspace/channel, so
        # agent-initiated reactions can default to "the message being replied to".
        self._last_inbound_ts: Dict[Tuple[str, str], str] = {}
        ...
        # a failed summon is discard()ed so an invite-and-retry works.
        self._reaction_summon_dedup = MessageDeduplicator(max_size=5000)
        # Per-workspace/channel conversation type cache (im / mpim / channel)
        # so reaction-driven synthetic events key their session the same way a
        # real message in that conversation would.
        self._reaction_channel_type: Dict[Tuple[str, str], str] = {}
        ...
        # ts) onto the ORIGINAL reacted message, scoped by workspace.
        self._reaction_lifecycle_target: Dict[Any, str] = {}
        self._REACTION_LIFECYCLE_TARGET_MAX = 5000
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Reactions have two independent opt-in routing modes:
            # ``reaction_trigger_emojis`` summons the bot onto the reacted
            # message, while ``reaction_triggers`` forwards a compact
            # reaction:added/removed turn. A matching summon emoji owns the
            # agent turn so enabling both settings cannot create duplicate
            # responses; the generic reaction hook still fires for observers.
            # ``body`` carries the outer envelope team id that inner reaction
            # events often omit on multi-workspace installs.
            @self._app.event("reaction_added")
            async def handle_reaction_added(event, body, say):
                await self._dispatch_slack_reaction_event(event, body)

            @self._app.event("reaction_removed")
            async def handle_reaction_removed(event, body, say):
                await self._dispatch_slack_reaction_event(
                    event, body, removed=True
                )
    ...
    def _agent_reaction_team_id(
        self, chat_id: str, team_id: str = ""
    ) -> Tuple[str, Optional[str]]:
        """Resolve an agent reaction's workspace, failing closed if ambiguous."""
        if team_id:
            return str(team_id), None
        teams = getattr(self, "_channel_teams", {}).get(str(chat_id), set())
        if len(teams) > 1:
            return "", "Ambiguous Slack channel across workspaces; pass workspace scope."
        if len(teams) == 1:
            return str(next(iter(teams))), None
        mapped = getattr(self, "_channel_team", {}).get(str(chat_id))
        if mapped:
            return str(mapped), None
        if len(getattr(self, "_team_clients", {})) > 1:
            return "", "Slack workspace scope is required for agent reactions."
        return "", None
    ...
    async def add_reaction(
        ...
        team_id, scope_error = self._agent_reaction_team_id(chat_id, team_id)
        if scope_error:
        ...
        target_id = message_id or self._last_inbound_ts.get((team_id, chat_id))
        ts = self._reaction_lifecycle_ts(target_id, team_id)
        if not ts:
            return {
                "success": False,
        ...
        ok = await self._add_reaction(chat_id, ts, name, team_id)
    ...
    async def remove_reaction(
        ...
        if emoji:
            ...
            # No emoji named — remove whatever this workspace's bot reacted with.
            try:
                resp = await self._get_client(
                    chat_id, team_id=team_id or None
                ).reactions_get(channel=chat_id, timestamp=ts)
        ...
        removed = 0
        for name in names:
            if name and await self._remove_reaction(chat_id, ts, name, team_id):
                removed += 1
    ...
    def _slack_reaction_status_emojis(self) -> dict:
        ...
        Configurable via ``slack.reaction_status_emojis`` (a mapping, JSON, or a
        positional CSV) or ``SLACK_REACTION_STATUS_EMOJIS``, merged over the defaults
        ``eyes`` / ``white_check_mark`` / ``x``. Cached — read on every
        processed message.
        """
        cached = getattr(self, "_reaction_status_emojis_cache", None)
        if cached is not None:
            return cached
        raw = self.config.extra.get("reaction_status_emojis") if self.config.extra else None
        if raw is None:
            raw = os.getenv("SLACK_REACTION_STATUS_EMOJIS", "")
        emojis = _parse_reaction_status_emojis(raw)
        self._reaction_status_emojis_cache = emojis
        return emojis

    # ----- Reaction triggers (emoji summon) -----

    def _slack_reaction_trigger_emojis(self) -> set:
        """Return emoji names (without colons) that summon the bot when used
        as reactions.
    ...
    def _parse_reaction_event(
        ...
        team_id = (
            self._event_team_id(event, body)
            or self._channel_team.get(channel_id)
            or ""
        )
        if not team_id and len(getattr(self, "_team_clients", {})) > 1:
            logger.warning(
                "[Slack] Dropping reaction event without workspace scope for %s",
                channel_id,
            )
            return None
        if team_id:
    ...
    async def _handle_slack_reaction_added(
        ...
        is_bot_reactor = await self._reactor_is_bot(
            ctx.user_id, ctx.channel_id, ctx.team_id
        )
        ...
        original, error_detail = await self._fetch_slack_message_for_reaction(
            ctx.channel_id, ctx.message_ts, ctx.team_id
        )
        if original is None:
            # Most fetch failures are access problems (not_in_channel,
            # channel_not_found, missing_scope). Leave a clear note in the
            # thread; if we cannot even post there, log and give up quietly.
            # Discard the dedup key so the retry we just told the user to do
            ...
            try:
                await self._get_client(
                    ctx.channel_id, team_id=ctx.team_id or None
                ).chat_postMessage(
                    channel=ctx.channel_id,
        ...
        synthetic_event = self._build_reaction_synthetic_event(
            ...
            channel_type=await self._resolve_reaction_channel_type(
                ctx.channel_id, ctx.team_id
            ),
        ...
        if synthetic_event.get("ts"):
            marker = self._workspace_message_marker(
                ctx.team_id, synthetic_event["ts"]
            )
            self._reaction_lifecycle_target[marker] = ctx.message_ts
            self._trim_oldest_dict_entries(
                self._reaction_lifecycle_target,
                self._REACTION_LIFECYCLE_TARGET_MAX,
            )
    ...
    async def _handle_slack_reaction_removed(
        ...
        if (
            await self._reactor_is_bot(ctx.user_id, ctx.channel_id, ctx.team_id)
    ...
    def _slack_thread_session_is_active(
        self,
        *,
        channel_id: str,
        thread_ts: str,
        ...
        """Return whether the Slack thread maps to an in-flight adapter turn.

        ``/stop`` is only meaningful while a turn is running, so this checks
        ``_active_sessions`` (the interrupt-guard map populated for the
        duration of a turn). The session key MUST be built with the same
        isolation settings the gateway used when it acquired the guard —
        those live on the session store's config, not ``self.config.extra``
        (``group_sessions_per_user`` is a gateway-global). Reading them from
        the wrong place silently mis-keys and the stop never matches.
        """
        ...
        try:
            ...
            return session_key in active_sessions
        except Exception:
            logger.debug(
                "[Slack] Failed to resolve reaction removal session key",
                exc_info=True,
            )
            return False

    def _session_isolation_settings(self) -> Tuple[bool, bool]:
        """Return ``(group_sessions_per_user, thread_sessions_per_user)``.

        Prefers the live session store's config — the single source of truth
        the gateway keys sessions with — falling back to platform ``extra``
        and finally the framework defaults. Mirrors
        ``_has_active_session_for_thread``.
        """
        store_cfg = getattr(getattr(self, "_session_store", None), "config", None)
        if store_cfg is not None:
            return (
                bool(getattr(store_cfg, "group_sessions_per_user", True)),
                bool(getattr(store_cfg, "thread_sessions_per_user", False)),
            )
        extra = self.config.extra or {}
        return (
            bool(extra.get("group_sessions_per_user", True)),
            bool(extra.get("thread_sessions_per_user", False)),
        )

    async def _resolve_reaction_channel_type(
        self, channel_id: str, team_id: str = ""
    ) -> str:
        """Resolve a workspace/channel's type for a reaction-driven event.

        Returns one of ``"im"`` / ``"mpim"`` / ``"channel"`` so a synthetic
        ...
        group DMs), so this consults ``conversations.info`` once per workspace
        and channel. Only a successful lookup is cached.
        """
        cache_key = (str(team_id or ""), str(channel_id))
        cached = self._reaction_channel_type.get(cache_key)
        if cached is not None:
            return cached
        ...
        try:
            ...
            if resp.get("ok", True):
                ...
                if chan.get("is_im"):
                    ...
                    resolved = "channel"
                self._reaction_channel_type[cache_key] = resolved
                return resolved
            logger.debug(
                "[Slack] conversations.info not ok for %s/%s: %s",
                team_id,
                channel_id,
                resp.get("error"),
            )
            ...
            logger.debug(
                "[Slack] conversations.info failed for %s/%s: %s",
                team_id,
                channel_id,
                exc,
            )
    ...
    async def _reactor_is_bot(
        self, user_id: str, channel_id: str = "", team_id: str = ""
    ) -> bool:
        """Resolve bot identity through the canonical workspace-aware cache."""
        return await self._resolve_user_is_bot(user_id, channel_id, team_id)

    async def _fetch_slack_message_for_reaction(
        self, channel_id: str, message_ts: str, team_id: str = ""
    ) -> Tuple[Optional[dict], str]:
        """Fetch the message a reaction points at.

        Returns ``(message, "")`` on success or ``(None, error_detail)`` when
    ...
    def _reaction_lifecycle_ts(
        self, message_id: Optional[str], team_id: str = ""
    ) -> Optional[str]:
        ...
        return getattr(self, "_reaction_lifecycle_target", {}).get(marker, message_id)
    ...
    async def on_processing_start(self, event: MessageEvent) -> None:
        ...
        if channel_id:
            react_ts = self._reaction_lifecycle_ts(ts, team_id) or ts
            await self._add_reaction(
                channel_id,
                react_ts,
                self._slack_reaction_status_emojis()["in_progress"],
                team_id,
            )
    ...
    async def on_processing_complete(
        ...
        team_id = str(getattr(event.source, "scope_id", "") or "")
        marker = self._workspace_message_marker(team_id, ts) if ts else None
        tracked = bool(ts and marker in self._reacting_message_ids)
        ...
        react_ts = self._reaction_lifecycle_ts(ts, team_id) if ts else ts
        if marker:
            self._reaction_lifecycle_target.pop(marker, None)
        if tracked:
            self._reacting_message_ids.discard(marker)
        if not self._reactions_enabled() or not tracked or not react_ts:
            return
        channel_id = getattr(event.source, "chat_id", None)
        if not channel_id:
            return
        ...
        await self._remove_reaction(
            channel_id, react_ts, emojis["in_progress"], team_id
        )
        if outcome == ProcessingOutcome.SUCCESS:
            await self._add_reaction(channel_id, react_ts, emojis["done"], team_id)
        elif outcome == ProcessingOutcome.FAILURE:
            await self._add_reaction(channel_id, react_ts, emojis["failed"], team_id)
    ...
    async def _dispatch_slack_reaction_event(
        self,
        event: dict,
        body: Optional[dict] = None,
        *,
        ...
        """Dispatch one Slack reaction without creating duplicate agent turns.

        ``reaction_trigger_emojis`` owns matching reactions because its
        message-fetching summon is richer than the compact
        ``reaction_triggers`` event turn. The generic path still runs with
        agent routing disabled so gateway reaction hooks observe the event.
        """
        summon_match = self._parse_reaction_event(event, body) is not None
        await self._handle_slack_reaction(
            event,
            body,
            removed=removed,
            route_to_agent=not summon_match,
        )
        if not summon_match:
            return
        if removed:
    ...
    async def _handle_slack_reaction(
        ...
        *,
        ...
        # from every connected workspace bot identity.
        bot_user_ids = {
            uid
            for uid in [
                self._bot_user_id,
                *getattr(self, "_team_bot_user_ids", {}).values(),
            ]
            if uid
        }
        ...
        if not team_id and len(getattr(self, "_team_clients", {})) > 1:
            logger.warning(
                "[Slack] Dropping generic reaction without workspace scope for %s",
                channel_id,
            )
            return
        if team_id:
        ...
        # A matching ``reaction_trigger_emojis`` summon owns the agent turn.
        # Event registration calls us with routing disabled in that case so
        # observers still receive the hook without a duplicate generic turn.
    ...
    async def _handle_slack_message(
        ...
        if (
            channel_id
            and ts
            and not event.get("_reaction_synthetic")
            and not event.get("_hermes_reaction")
        ):
            self._last_inbound_ts[(str(team_id or ""), channel_id)] = ts
...
def _parse_reaction_trigger_emojis(raw) -> set:
    ...
    emojis = set()
    for entry in raw:
        name = str(entry).strip().strip(":").lower()
        if name:
            emojis.add(name)
    return emojis


_DEFAULT_REACTION_STATUS_EMOJIS = {
    "in_progress": "eyes",
...
def _parse_reaction_status_emojis(raw) -> dict:
    """Normalise a ``reaction_status_emojis`` config value into a full emoji mapping.
    ...
    """
    result = dict(_DEFAULT_REACTION_STATUS_EMOJIS)
    if raw is None:
        return result
    if isinstance(raw, str):
        s = raw.strip()
...
def _apply_yaml_config(yaml_cfg: dict, slack_cfg: dict) -> dict | None:
    ...
    re_cfg = slack_cfg.get("reaction_status_emojis")
    if re_cfg is not None and not os.getenv("SLACK_REACTION_STATUS_EMOJIS"):
        os.environ["SLACK_REACTION_STATUS_EMOJIS"] = json.dumps(_parse_reaction_status_emojis(re_cfg))
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # reactions, agent_reactions, reaction_trigger_emojis,
        # reaction_status_emojis, reaction_triggers,
        # reaction_trigger_target, disable_dms, allowed_channels, and
        # ignored_channels) into SLACK_* env vars that the adapter reads via
        # os.getenv(). Replaces the
